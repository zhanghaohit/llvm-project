...
void SelectionDAGBuilder::visitGetElementPtr(const User &I) {
  ...
  for (gep_type_iterator GTI = gep_type_begin(&I), E = gep_type_end(&I);
    ...
    if (StructType *StTy = GTI.getStructTypeOrNull()) {
      // Guard against non-constant struct index (can be generated by obfuscation
      // passes). Valid IR requires constant indices for struct GEPs, but variable
      // indices on array-of-structs are valid pointer arithmetic that may slip through.
      // Use break (not continue) because ++GTI would call getTypeAtIndex(variable) → crash.
      auto *IdxC = dyn_cast<Constant>(Idx);
      if (!IdxC)
        break;
      unsigned Field = IdxC->getUniqueInteger().getZExtValue();
      if (Field) {
        // N = N + Offset
        uint64_t Offset =
      ...
      // Guard against null StructType* CurTy (from iterating past a scalar
      // element type). getIndexedType() would dereference null in that case.
      if (!GTI.isSequential())
        continue;
